...
const MeditationSessionScreen = () => {
  ...
  const { selectedType, selectedDuration, submitMeditationSession, useStone } = useMeditationStore();
  ...
  const startSession = async () => {
    // === Use Stone Logic Start ===
    const stoneUsed = useStone();
    if (!stoneUsed) {
      Alert.alert(
        "Out of Focus Stones",
        "You need a Focus Stone to begin meditation. Stones refill daily.",
        [{ text: "OK", onPress: () => navigation.goBack() }]
      );
      return; // Stop execution if no stone was used
    }
    console.log('[Analytics] Event: stone_success'); // Log stone usage
    // === Use Stone Logic End ===
